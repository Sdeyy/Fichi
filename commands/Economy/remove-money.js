...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        if(data.money < amount) return interaction.reply({content: 'User doesn\'t have enough money!'});

        await ecoSchema.findOneAndUpdate({userID: user.id}, {
            $inc: {money: -amount}
        });

        interaction.reply({content: `Removed ${amount} coins from ${user.tag}'s wallet!`});
    }
}
